class RecorderManager {
    ...
    getRecorder(ready) {
        return new Promise((resolve, reject)=>{
            if (!this.input || !this.recorder) navigator.getUserMedia({
                audio: true
            }, (stream)=>{
    ...
    async start() {
        ...
        } else throw new Error("Already recording");
    }
    async stop() {
        if (this.isRecording) {
            const recorder = await this.getRecorder();
        ...
        } else throw new Error("Not Recording");
    }
    createDownloadLink() {
...
//# sourceMappingURL=index.8c63b8a7.js.map
